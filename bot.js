...
client.on('guildMemberAdd', member => {
  member.guild.fetchInvites().then(guildInvites => {
    ...
    const welcome = member.guild.channels.find(channel => channel.name === "𝑾𝒆𝒍𝒄𝒐𝒎𝒆☃");
    welcome.send(`** ${member} joined  invited by ${inviter}   (  ${invite.uses} invites ) ** `)
  });
});
...
.setColor("#000000")
.addField(`ᵛᵉʳⁱᶠⁱᵉᵈ  
Your invite codes in ${message.guild.name}`, ` Invite Codes :\n${codes.join("\n")}`)
.setColor('#36393e')
message.channel.send({ embed: embed });
return;
}
...
if(!message.member.hasPermission('MANAGE_CHANNELS')) return message.reply
         ...
         SEND_MESSAGES: false
               ...
               message.reply("** ✖️ | امر وطاعه يامولاي الشات مغلق  **")
           });
             }
...
client.on('message', async message => {
 if(message.channel.type === "dm") return;
  ...
  // const wlcImage = await fs.readFile("./welcome111.png"); //اسم الصورة
